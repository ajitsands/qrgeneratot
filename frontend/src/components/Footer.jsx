...
export default function Footer() {
  ...
  const [activeView, setActiveView] = useState('main'); // 'main' or 'products'
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (showPopup && activeView === 'products' && products.length === 0) {
      fetchProducts();
    }
  }, [showPopup, activeView]);
  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 w-full p-4 text-center z-40">
        <button
          onClick={() => { setShowPopup(true); setActiveView('main'); }}
              ...
              <div className="flex items-center gap-3">
                {activeView === 'products' && (
                  <button 
                    ...
                    className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors bg-white/50 dark:bg-black/20 rounded-full flex-shrink-0"
                  >
                ...
                <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-blue-500 dark:from-purple-400 dark:to-blue-400">
                  {activeView === 'main' ? 'Discover SaNDS Lab' : 'Our Latest Products'}
                </h2>
              </div>
              <button 
            ...
            {activeView === 'main' ? (
              <div className="p-6 flex flex-col gap-4">
                <a 
                  ...
                  className="w-full py-4 px-4 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-xl text-center font-semibold hover:bg-blue-100 dark:hover:bg-blue-800/40 transition-colors flex items-center justify-center gap-2"
                >
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="w-full py-4 px-4 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-xl text-center font-semibold hover:bg-green-100 dark:hover:bg-green-800/40 transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button 
                  ...
                  className="w-full py-4 px-4 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-xl text-center font-semibold hover:bg-purple-100 dark:hover:bg-purple-800/40 transition-colors flex items-center justify-center gap-2"
                >
